# NEFROLITOTOMIA (RETIRADA CIRÚRGICA DE CÁLCULO RENAL)

> Fonte: 19_NEFROLITOTOMIA.docx · termo_consentimento · ASSINATURA · 46 parágrafos

3. O QUE É O PROCEDIMENTO

A nefrolitotomia é a remoção de cálculo(s) (pedras) localizado(s) no interior do rim, mediante acesso cirúrgico ao órgão — em regra por incisão lombar (lombotomia), com abertura do parênquima renal e/ou do sistema coletor para extração dos cálculos —, podendo ser deixados, ao final, dreno e/ou cateter duplo J (cateter interno entre o rim e a bexiga) para adequada cicatrização e drenagem urinária.

4. INDICAÇÕES

Cálculos renais volumosos, coraliformes ou complexos, não passíveis de tratamento por métodos menos invasivos, ou após falha destes; cálculos associados a alterações anatômicas que exijam correção cirúrgica simultânea.
...
Sangramento renal, intra ou pós-operatório, com eventual necessidade de transfusão, embolização ou, em casos extremos, nefrectomia (remoção do rim) para controle de hemorragia;

Fístula urinária (vazamento persistente de urina), em regra tratada com drenagem e cateter duplo J;

Cálculos residuais: fragmentos podem permanecer e exigir procedimentos complementares (litotripsia, endoscopia ou nova cirurgia);

Infecção urinária, podendo evoluir para infecção generalizada (urossepse);

Lesão da pleura com pneumotórax, podendo exigir drenagem torácica;

Perda de função do rim operado, parcial ou, raramente, total;

Estenose (estreitamento cicatricial) do sistema coletor;

Recidiva da doença litiásica (formação de novos cálculos), sendo essencial o seguimento clínico-metabólico;

Desconforto relacionado ao cateter duplo J (urgência miccional, sangue na urina, dor lombar ao urinar) até sua retirada.

8. ALTERNATIVAS TERAPÊUTICAS

Nefrolitotripsia percutânea (acesso por punção), ureterorrenolitotripsia flexível a laser, litotripsia extracorpórea por ondas de choque (LECO), conforme características do cálculo e disponibilidade técnica. A não remoção de cálculos volumosos pode levar à perda do rim e a infecções graves.

9. CUIDADOS E RECOMENDAÇÕES PÓS-OPERATÓRIAS

Hidratação abundante, filtração da urina para resgate de fragmentos (quando orientado), cuidados com dreno e sonda, retorno para retirada do duplo J — ATENÇÃO: o cateter duplo J NÃO pode ser esquecido, devendo ser removido no prazo orientado, sob pena de complicações graves (incrustação, infecção, perda renal). Sinais de alerta: febre, dor lombar intensa, sangramento urinário volumoso.
...
Assim, de forma LIVRE, CONSCIENTE e ESCLARECIDA, AUTORIZO a realização do procedimento NEFROLITOTOMIA (RETIRADA CIRÚRGICA DE CÁLCULO RENAL), pela equipe acima identificada, nas dependências do Hospital das Clínicas de Mineiros Ltda.
